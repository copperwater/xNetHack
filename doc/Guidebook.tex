...
enabled by setting the correspondingly named option in {\tt XNETHACKOPTIONS}
...
file, or in the ``{\tt XNETHACKOPTIONS}'' environment variable.
...
text editor.
After running {\it xNetHack\/} for the first time, you should find a default
template for ths configuration file named \mbox{``xnethackrc.template''} in
\mbox{{``\%USERPROFILE\%\textbackslash xNetHack\textbackslash''}}.
If you have not created the configuration file, {\it xNetHack\/} will create
...
%.hn 2
\subsection*{Using the {\tt XNETHACKOPTIONS} environment variable}
...
\verb@+@ & S\verb+_+vcidoor                 &	(closed iron door)\\
...
\verb@-@ & S\verb+_+voidoor                 &	(open iron door)\\
