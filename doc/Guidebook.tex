...
enabled by setting the correspondingly named option in \texttt{XNETHACKOPTIONS}
...
file, or in the ``\texttt{XNETHACKOPTIONS}'' environment variable.
...
%.lp ""
On Windows, the name is \mbox{``.xnethackrc''} location in the folder
\mbox{{``\%USERPROFILE\%\textbackslash xNetHack\textbackslash''}}.
The file may not exist,
but it is a normal ASCII text file and can be created with any
text editor.
After running \textit{xNetHack} for the first time, you should find a default
template for ths configuration file named \mbox{``.xnethackrc.template''} in
\mbox{{``\%USERPROFILE\%\textbackslash xNetHack\textbackslash''}}.
If you have not created the configuration file, \textit{xNetHack} will create
...
%.lp
\item[HACKDIR]
Default location of files \textit{xNetHack} needs. On Windows HACKDIR
defaults to the location of the \textit{xNetHack.exe} or \textit{xNetHackw.exe} file
...
%.hn 2
\subsection*{Using the \texttt{XNETHACKOPTIONS} environment variable}
...
%.pg
The XNETHACKOPTIONS value is effectively the same as a single OPTIONS
...
XNETHACKOPTIONS can be set to the full name of a configuration file you
want to use.
...
\\
%note: 'A' and 'C' can be set in RC file or XNETHACKOPTIONS but not by 'O'
% \texttt{A} ---
...
non-comment line.
For a comma-separated list in XNETHACKOPTIONS or an OPTIONS line in a
...
\textit{statuslines}
can only be set in the run-time configuration file or via XNETHACKOPTIONS,
...
You can also override one or more symbols using the \textit{SYMBOLS}
...
+ & S\textunderscore vcidoor                 &	(closed iron door)\\
...
- & S\textunderscore voidoor                 &	(open iron door)\\
