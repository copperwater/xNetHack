...
staticfn int count_monstercolors(void);
...
staticfn int handler_monstercolor(void);
    ...
    if (req == do_set) {
        ...
        if (opts[0] < ' ') {
            ...
            /* for 'initial', update of BOULDER symbol is done in
               ...
               have been processed */
            if (!go.opt_initial) {
                nhsym sym = get_othersym(SYM_BOULDER, PRIMARYSET);
...
static int
optfn_orientation(int optidx, int req, boolean negated, char *opts, char *op)
{
    if (req == do_init) {
        return optn_ok;
    }
...
static int
optfn_polyinit(int optidx, int req, boolean negated, char *opts, char *op)
{
    nhUse(negated);
    if (req == do_init) {
        return optn_ok;
        ...
        int monnum = name_to_mon(op, (int *) 0);
        struct permonst *mdat;
        if (monnum < LOW_PM) {
            config_error_add("%s: no such monster '%s'.",
                             allopt[optidx].name, op);
            return optn_err;
        }
        mdat = &mons[monnum];
        if (unique_corpstat(mdat)) {
            config_error_add("cannot polyinit into unique monster '%s'.", op);
            return optn_err;
        }
        else if (is_mplayer(mdat)) {
            config_error_add("cannot polyinit into player monster '%s'.", op);
            return optn_err;
        }
        ...
        if (!opts)
            return optn_err;
        Sprintf(opts, "%s", mons[flags.polyinit_mnum].pmnames[NEUTRAL]);
        return optn_ok;
    }
    return optn_ok;
}
...
staticfn int
handler_symset(int optidx UNUSED)
{
    int reslt;

    reslt = do_symset();   /* symbols.c */
    go.opt_need_redraw = TRUE;
    return reslt;
}
...
handler_monstercolor(void)
{
    ...
    do {
        ...
        if (opt_idx == 3) { /* done */
             ...
             * monsters */
            color = query_color((const char *) 0, NO_COLOR);
            ...
            if (nummoncolors) {
                ...
                for (mndx = LOW_PM; mndx < NUMMONS; ++mndx) {
                    if (gm.monstercolors[mndx] != MONSTERCOLOR_DEFAULT) {
                        glyph_info info;
                        map_glyphinfo(0, 0, monnum_to_glyph(mndx, MALE), 0, &info);
                        any.a_int = mndx;
            ...
            if (pick_cnt > 0) {
                for (pick_idx = 0; pick_idx < pick_cnt; ++pick_idx) {
                    ...
                    gm.monstercolors[mndx] = MONSTERCOLOR_DEFAULT;
                }
                free((genericptr_t) pick_list), pick_list = (menu_item *) 0;
            }
...
initoptions_finish(void)
{
    ...
    sym = get_othersym(SYM_BOULDER, PRIMARYSET);
    if (sym)
        gs.showsyms[SYM_BOULDER + SYM_OFF_X] = sym;
    reglyph_darkroom();
...
add_monstercolor(char *str)
{
    ...
    *colon = '\0'; /* terminate monster string */
    ...
    color = match_str2clr(colorstr, FALSE); /* if invalid color, prints error
                                               and returns CLR_MAX */
    if (color == CLR_MAX) {
        return FALSE;
    }
...
count_monstercolors(void)
{
    ...
    for (i = LOW_PM; i < NUMMONS; i++) {
        if (gm.monstercolors[i] != MONSTERCOLOR_DEFAULT) {
            numcolors++;
        }
    }
    return numcolors;
