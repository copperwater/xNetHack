...
staticfn struct obj * find_sink_ring(coordxy, coordxy);
...
drinkfountain(void)
{
    ...
    if (mgkftn && !Doomed && u.uluck >= 0 && fate >= 10) {
        if (u.uhp < u.uhpmax * 3 / 5 || u.uen < u.uenmax * 3 / 5) {
            /* Healing spring */
            pline_The("water heals and reenergizes you!");
...
dipfountain(struct obj *obj)
{
    ...
    if (obj->otyp == LONG_SWORD
        && u.ulevel >= ((Role_if(PM_KNIGHT) && !u.ualign.abuse) ? 5 : 10)
...
/* If there is a ring buried under a sink located at (sink_x, sink_y), return a
 * pointer to it, but do nothing else.
 * If multiple rings, return the first of them. */
staticfn struct obj *
find_sink_ring(coordxy sink_x, coordxy sink_y)
{
    struct obj *otmp;
...
struct obj *
ring_from_sink(coordxy sink_x, coordxy sink_y)
{
    struct obj* otmp = find_sink_ring(sink_x, sink_y);
    if (otmp) {
...
dipsink(struct obj *obj)
{
    boolean try_call = FALSE,
            not_looted_yet = !!find_sink_ring(u.ux, u.uy),
    ...
    case POT_OBJECT_DETECTION:
        if (not_looted_yet) {
            ...
            makeknown(POT_OBJECT_DETECTION);
            break;
        }
        FALLTHROUGH;
        /* FALLTHRU */
