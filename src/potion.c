...
static void FDECL(dipsink, (struct obj *));
...
struct obj *obj;
{
    ...
    if (obj == &cg.zeroobj && can_reach_floor(FALSE) &&
...
struct obj *obj;
{
    if (obj == &cg.zeroobj) {
        ...
        /* shouldn't be possible to be levitating over a sink anyway */
...
register struct obj *obj;
{
    ...
    boolean breathe = !breathless(g.youmonst.data);
    boolean cansmell = breathe && olfaction(g.youmonst.data);
    boolean eyes = eyecount(g.youmonst.data);
    const char * eyestr =
...
dodip()
{
    ...
    Sprintf(qbuf, "dip %s into", flags.verbose ? yname(obj) : shortestname);
