...
static struct obj *FDECL(wizterrainwish, (char *, char *, XCHAR_P));
...
static struct obj *
wizterrainwish(bp, p, doorflags)
char *bp, *p;
xchar doorflags; /* contains all flags, including locked and trapped */
{
    struct rm *lev;
    ...
    if (!BSTRCMPI(bp, p - 4, "door")) {
        boolean secret = !strncmpi(bp, "secret ", 7);

        if (secret && lev->typ != DOOR && lev->typ != SDOOR
            ...
            /* Secret doors want both doormask and wall_info but those both
             * overload rm.flags which makes D_LOCKED conflict with WM_MASK.
             * The extent of this causing problems is unknown. */
            lev->typ = secret ? SDOOR : DOOR;
            set_doorstate(lev, doorflags); /* will set only state */
            set_door_lock(lev, ((doorflags & D_LOCKED) != 0));
            set_door_trap(lev, ((doorflags & D_TRAPPED) != 0));
            set_door_iron(lev, ((doorflags & D_IRON) != 0));

            pline("A %s%sdoor%s.", ((doorflags & D_TRAPPED) ? "trapped " : ""),
                  (secret ? "secret " : ""),
                  (doorstate(lev) == D_NODOOR ? "way" : ""));
            madeterrain = TRUE;
        }
    ...
    else if (!BSTRCMPI(bp, p - 8, "fountain")) {
        lev->typ = FOUNTAIN;
...
struct obj *no_wish;
{
 ...
 wiztrap:
    if (wizard && !g.program_state.wizkit_wishing) {
        ...
        if (d.material == IRON)
            doorflags |= D_IRON;
        if ((d.otmp = wizterrainwish(d.bp, d.p, d.doorstate)) != 0)
            return d.otmp;
    }
 ...
 typfnd:
    ...
    if (d.islit && (d.typ == OIL_LAMP || d.typ == MAGIC_LAMP || d.typ == LANTERN
