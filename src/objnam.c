    ...
    unsigned doname_flags) /* special case requests */
{
    ...
    case ARMOR_CLASS:
        ...
        if (Is_dragon_scaled_armor(obj)) {
            Sprintf(eos(prefix), "%s-scaled ", dragon_scales_color(obj));
        }
        FALLTHROUGH;
        /*FALLTHRU*/
    case WEAPON_CLASS:
 ...
 ring:  /* normal rings reach here 'naturally'; meat ring jumps here */
    ...
    case GEM_CLASS:
        if (obj->otyp == THIEFSTONE && known) {
            if (!thiefstone_ledger_valid(obj)) {
                ...
                Concat(bp, 0, " (inactive)");
            } else {
                d_level dlev;
                dlev.dnum = ledger_to_dnum(obj->keyed_ledger);
                dlev.dlevel = ledger_to_dlev(obj->keyed_ledger);
                ConcatF2(bp, 0, " (keyed to %s:%d)",
                         svd.dungeons[dlev.dnum].dname, depth(&dlev));
                if (wizard) {
                    ConcatF2(bp, 0, " at (%d,%d)",
    ...
    if (iflags.invweight && (obj->where == OBJ_INVENT || wizard)) {
        /* user has asked to see object weights */
...
not_fully_identified(struct obj *otmp)
{
    ...
    if (otmp->rknown
        ...
        return (boolean) (is_damageable(otmp)
                          ...
                          /* is_crackable() is covered in is_damageable() but
                           * won't suffice here unless every item made of glass
                           * cracks before breaking, which light objects like
                           * glass wands won't; since those can be made
                           * indestructible, they are covered here */
                          || otmp->material == GLASS);
}
...
wizterrainwish(struct _readobjnam_data *d)
{
    ...
    for (trap = NO_TRAP + 1; trap < TRAPNUM; trap++) {
        struct trap *t;
        const char *tname = trapname(trap, TRUE);
    ...
    if (!BSTRCMPI(bp, p - 8, "fountain")) {
        ...
        lev->looted = d->looted ? (S_LPUDDING | S_LDWASHER) : 0;
        /* wished-for sinks never have a buried ring regardless of d->looted */
        pline("A sink.");
        madeterrain = TRUE;
    ...
    /* also matches "molten lava" */
        ...
        boolean irondoor = d->material == IRON;
        ...
        if (lev->typ == DOOR || lev->typ == SDOOR
            ...
            /* secret doors mustn't be set locked explicitly because they use
               both doormask and wall_info which both overload rm[x][y].flags
               (LOCKED and door state overlap wall_info bits, IRON and TRAPPED
               don't); conversion from SDOOR to DOOR changes NODOOR to CLOSED */
            set_doorstate(lev, ((d->doorless || secret) ? D_NODOOR
                                 : d->open ? D_ISOPEN
                                   : d->broken ? D_BROKEN
                                     : D_CLOSED));
            set_door_lock(lev, (!secret && d->locked));
             ...
             * FIXME: certain xNetHack traps might be able to activate when
             * interacting with an open door. This ideally should check to see
             * if the type of trap is like that. */
            if (d->trapped == 2 /* 2: wish includes explicit "untrapped" */
                ...
                || doorstate(lev) == D_NODOOR || doorstate(lev) == D_ISOPEN)
                d->trapped = 0;
            set_door_trap(lev, d->trapped);
            /* set up iron door flag */
            set_door_iron(lev, irondoor);

            /* feedback */
            dbuf[0] = '\0';
            if (door_is_trapped(lev))
                Strcat(dbuf, "trapped ");
            if (door_is_locked(lev))
                Strcat(dbuf, "locked ");
            if (door_is_iron(lev))
                Strcat(dbuf, "iron ");
            if (lev->typ == SDOOR) {
                ...
                if (doorstate(lev) == D_CLOSED)
                    Strcat(dbuf, "closed ");
                if (doorstate(lev) == D_ISOPEN)
                    Strcat(dbuf, "open ");
                if (doorstate(lev) == D_BROKEN)
                    Strcat(dbuf, "broken ");
                if (doorstate(lev) == D_NODOOR)
    ...
    for (;;) {
        ...
        if (!strncmpi(d->bp, "an ", l = 3) || !strncmpi(d->bp, "a ", l = 2)) {
            ...
            d->uncursed = 1, d->blessed = d->iscursed = 0;
                   ...
                   /* prior to upstream NetHack introducing "tempered" in 2024,
                    * xNetHack used "shatterproof" for the same purpose; allow
                    * this as an alias to support players used to wishing for
                    * it. */
        ...
        /* looted: fountain/sink/throne/tree; disturbed: grave */
            ...
            d->doorstate = D_CLOSED;
        } else if (!strncmpi(d->bp, "open ", l = 5)) {
            d->doorstate = D_ISOPEN;
...
readobjnam(char *bp, struct obj *no_wish)
{
 ...
 typfnd:
     ...
     * (won't be clearing an artifact name since mkobj/mksobj was called with
     * artif=FALSE above) */
    free_oname(d.otmp);
