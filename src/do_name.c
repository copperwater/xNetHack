...
do_oname(struct obj *obj)
{
    ...
    if ((aname = artifact_name(buf, &objtyp, TRUE)) != 0) {
        if (restrict_name(obj, aname) || exist_artifact(obj->otyp, aname)) {
...
weapon_oname(struct obj *wpn)
{
    ...
    const char* wpn_names[] = {
        ...
        "%s of Quarrels", "%s of Grace",   "%s of Respect",  "%s of Paranoia",
        "%s of Fury",     "%s of Hubris",  "%s of Insanity",
        ...
        "Elegant %s",     "Artful %s",     "Wicked %s",      "Clever %s",
    ...
    const char* name = ROLL_FROM(wpn_names);
    if (strstri(name, "%s")) {
...
mintroduce(struct monst *mtmp)
{
    ...
    if (!has_mgivenname(mtmp)) {
        ...
        if (mtmp->data->mlet == S_NYMPH) {
            name = ROLL_FROM(nymphnames);
        }
        else if (is_demon(mtmp->data)) {
            if (mtmp->female)
                name = ROLL_FROM(femdemonnames);
            else
                name = ROLL_FROM(maldemonnames);
        }
        else {
