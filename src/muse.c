...
staticfn boolean wont_leave_level(struct monst *);
    ...
    for (obj = mtmp->minvent; obj; obj = obj->nobj) {
        if (!reflection_skip) {
            ...
            if (obj->otyp == WAN_MAGIC_MISSILE && obj->spe > 0
                /* In xNetHack, Antimagic doesn't protect completely from magic
                 * missiles, so this shouldn't be a factor
                 * && !m_seenres(mtmp, M_SEEN_MAGR) */
                ) {
                gm.m.offensive = obj;
    ...
    struct obj *obj)              /* 2nd arg to fhitm/fhito */
{
    ...
    while (range-- > 0) {
        ...
        if (otyp == WAN_STRIKING
            ...
            case WAN_LOCKING:
            case WAN_STRIKING:
                if (doorlock(obj, mon, gb.bhitpos.x, gb.bhitpos.y)) {
                    if (gz.zap_oseen)
                        makeknown(otyp);
        ...
        if (!ZAP_POS(ltyp)
            || (IS_DOOR(ltyp)
                && door_is_closed(&levl[gb.bhitpos.x][gb.bhitpos.y]))) {
            gb.bhitpos.x -= ddx;
            gb.bhitpos.y -= ddy;
            break;
        }
...
#define nomore(x)       if (gm.m.has_misc == (x)) continue
    ...
    for (obj = mtmp->minvent; obj; obj = obj->nobj) {
        ...
        if (obj->oartifact == ART_ITLACHIAYAQUE && obj->age <= svm.moves
            && !mtmp->mpeaceful && !mtmp->mblinded && m_canseeu(mtmp)) {
            gm.m.misc = obj;
            gm.m.has_misc = MUSE_ITLACHIAYAQUE;
        }
    }
...
use_misc(struct monst *mtmp)
{
 ...
 skipmsg:
    ...
    case MUSE_ITLACHIAYAQUE:
        ...
        otmp->age = svm.moves + (mtmp->data == &mons[PM_SCHLIEMANN]
                                      ? rn1(20,10) : rnz(100));
        if (cansee(mtmp->mx, mtmp->my)) {
        ...
        else if (!breathless(gy.youmonst.data)) {
