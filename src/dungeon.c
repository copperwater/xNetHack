...
get_dgn_flags(lua_State *L)
{
    ...
    static const char *const flagstrs[] = {
        "town", "hellish", "mazelike", "unconnected", NULL
    };
    static const int flagstrs2i[] = {
        TOWN, HELLISH, MAZELIKE, UNCONNECTED, 0
    };
...
u_on_newpos(coordxy x, coordxy y)
{
    ...
    if (gm.multi < 0 && (!strcmp(gm.multi_reason, "moving through the air")))
        ; /* hurtling */
...
recalc_mapseen(void)
{
    ...
    mptr->flags.quest_summons = (at_dgn_entrance("The Quest")
                                 ...
                                 && !(u.uevent.qcompleted
                                      || u.uevent.qexpelled
                                      || svq.quest_status.killed_leader));
...
#ifdef DUMPHTML
# define dump_aware_add_menu(win, buf)                                  \
    do {                                                                \
        if (final > 0)                                                  \
            add_menu(win, &nul_glyphinfo, &any, 0, 0, ATR_PREFORM,      \
                     NO_COLOR, buf,                                     \
             MENU_ITEMFLAGS_SKIPINVERT | MENU_ITEMFLAGS_SKIPMENUCOLORS);\
        else                                                            \
            add_menu_str(win, buf);                                     \
    } while(0)
#else
# define dump_aware_add_menu(win, buf)                                  \
    do { add_menu_str(win, buf); } while(0)
#endif /* DUMPHTML */
    ...
    boolean printdun)
{
    ...
    *buf = '\0';
    ...
    /* quest entrance is not mutually-exclusive with bigroom */
...
/* Return true if there are undiscovered final resting places on the current
 * level. */
boolean
undiscovered_bones(void)
{
    struct mapseen *mptr = find_mapseen(&u.uz);
    struct cemetery *bp;
