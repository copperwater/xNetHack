...
#define KEEPTRAITS(mon)                                                  \
    ((mon)->isshk || (mon)->mtame || unique_corpstat((mon)->data)        \
     ...
     || (mon)->m_id == svq.quest_status.nemesis_m_id                      \
...
/*    int mndx = monsndx(mdat); */
    ...
    case PM_LEATHER_GOLEM:
        ...
        while (num--) {
            ...
            if (!valid_obj_material(obj, LEATHER)) {
                delobj(obj);
                obj = mksobj(rn2(4) ? LIGHT_ARMOR : SADDLE,
                             TRUE, FALSE);
            }
    ...
    case PM_TIGER:
        if (!mtmp->mrevived && !rn2(100)) {
            int otyp;
            for (otyp = svb.bases[RING_CLASS]; otyp < svb.bases[RING_CLASS+1];
                 ++otyp) {
                const char *s;
            ...
            if (otyp >= svb.bases[RING_CLASS + 1])
    ...
    case PM_PANTHER: case PM_LARGE_CAT:
    ...
    case PM_VIOLET_FUNGUS: case PM_BLACK_MOLD:
    ...
    case PM_LORD_SURTUR: case PM_ANARAXIS_THE_BLACK: case PM_STUDENT:
...
meatmetal(struct monst *mtmp)
{
    ...
    for (otmp = svl.level.objects[mtmp->mx][mtmp->my]; otmp;
        ...
        if (!can_eat_material(mtmp->data, otmp->material)
            ...
            || ((otmp->opoisoned || permapoisoned(otmp))
                && !resists_poison(mtmp)))
            continue;
        if (is_metallic(otmp) && !obj_resists(otmp, 5, 95)
            ...
            if (mtmp->data == &mons[PM_RUST_MONSTER] && otmp->oerodeproof) {
                ...
                /* Metal is slow to digest, and most metal objects will not have
                 * any special effects like polymorphing or leveling up the
                 * monster. Put it in the monster's "stomach" most of the time
                 * instead of consuming it - unless it is a tin, which is the
                 * one metal object that often does have special effects.
                 * This is not ideal - ideal would probably be to have
                 * m_consume_obj return different values based on whether
...
mon_allowflags(struct monst *mtmp)
{
    ...
    boolean can_tunnel = tunnels(mtmp->data);
    ...
    if (is_human(mtmp->data) || mtmp->data == &mons[PM_MINOTAUR])
        allowflags |= ALLOW_SSM;
    if ((is_undead(mtmp->data) && !noncorporeal(mtmp->data))
        || is_vampshifter(mtmp))
        allowflags |= NOGARLIC;
 ...
 * This does NOT refer to open air/chasms like with is_open_air!
 * It is almost equivalent with !grounded(mtmp->data), but not exactly because
 * of the mundetected check. */
boolean
        ...
        for (ny = max(0, y - 1); ny <= maxy; ny++) {
            ...
            if ((poolok || is_pool(nx, ny) == wantpool)
                ...
                if ((ttmp = t_at(nx, ny)) != 0) {
                    ...
                    else if (immune_to_trap(mon, ttmp->ttyp)) {
                        if (!(flag & ALLOW_TRAPS)) {
    ...
    boolean due_to_death)
{
    ...
    if (due_to_death) {
        if (mtmp->data->msound == MS_NEMESIS) {
            nemdead();
            /* The Caveman and Priest quest texts describe
...
boolean
faulty_lifesaver(struct obj* obj)
{
    if (!obj) {
...
vamprises(struct monst *mtmp)
{
    ...
    if (ismnum(mndx) && mndx != monsndx(mtmp->data)
        ...
        if (closed_door(x, y)) {
            ...
            boolean kaboom = (door_is_trapped(door)
                              && getdoortrap(x, y) == FIRE_BLAST);

            if (door_is_iron(door) && !kaboom) {
                coord newpos;
                enexto(&newpos, x, y, mtmp->data);
                rloc_to(mtmp, newpos.x, newpos.y);
                return TRUE;
            }

            set_msg_xy(x, y); /* You()/pline() will reset this */
            if (!cansee(x, y))
                You_hear(door_smashed);
            else if (!canspotmon(mtmp))
                You_see(door_smashed);
            else if (!Unaware)
                pline_The("door is smashed.");
...
logdeadmon(struct monst *mtmp, int mndx)
{
    ...
    if (mndx == PM_MEDUSA && howmany == 1) {
        ...
        if (howmany <= 3 || howmany == 5 || howmany == 10 || howmany == 25
            ...
            char xtra[BUFSZ]; /* space for " (Nth time)" when N > 1 */
            char wherebuf[100]; /* on what level? (for Wizard) */
            ...
            if (howmany > 1) /* "(2nd time)" or "(50th time)" */
                Sprintf(xtra, " (for the %d%s time%s)", howmany, ordin(howmany),
                        wherebuf);
    ...
    int xkill_flags) /* 1: suppress mesg, 2: suppress corpse, 4: pacifist */
{
    ...
    if (Is_geryon_level(&u.uz) && !svc.context.mon_moving
 ...
 cleanup:
    ...
    } else if (mtmp->m_id == svq.quest_status.nemesis_m_id) { /* Real good! */
...
mnexto(struct monst *mtmp, unsigned int rlocflags)
{
     ...
     * message */
    if (!gi.in_mklev && (mtmp->mstrategy & STRAT_APPEARMSG)
        && canspotmon(mtmp)
...
/* Nazgul special action: terrifying shriek */
staticfn void
m_respond_nazgul(struct monst *mtmp)
{
    boolean cansee = canseemon(mtmp);
    struct monst *bystander;

    if (mtmp->mcan || mtmp->mtame || mtmp->mspec_used != 0)
        return;

    /* mspec_used, which we increase in this function, also controls whether a
     * Nazgul's breath weapon is ready for use. This code gets called in dochug
     * before it attempts to use its attacks, so if it tried to shriek 100% of
     * the time, it would never use the breath weapon. Thus, only attempt to
     * shriek a certain amount of the time. */
    if (rn2(3))
        return;
    ...
    case PM_VAMPIRE_LEADER: /* vampire lord or Vlad can become wolf */
        if (!rn2(wolfchance)
            /* don't pick a walking form if that would lead to immediate
               drowning or immolation and reversion to vampire form, or
               plummeting into the depths */
            && !is_pool_or_lava(mon->mx, mon->my)
            && !is_open_air(mon->mx, mon->my)) {
            mndx = PM_WOLF;
            break;
        }
        FALLTHROUGH;
...
select_newcham_form(struct monst *mon)
{
    ...
    case PM_CHAMELEON:
        if (!rn2(3)) {
            const int maxdiff = monmax_difficulty(level_difficulty());
            tryct = 20;
            do {
                mndx = pick_animal();
            } while (--tryct > 0 && toostrong(mndx, maxdiff));
            if (tryct == 0) {
