...
#undef add_ac
    ...
    case S_GNOME:
        ...
        if (!In_mines(&u.uz)) {
            ...
            while (ngems > 0) {
                (void) mongets(mtmp,
                               rnd_class(FIRST_REAL_GEM, LAST_GLASS_GEM));
                ngems--;
            }
        }
        break;
    ...
    *m2 = *mon; /* copy condition of old monster */
    ...
    if (m2->isminion) {
        ...
        *EMIN(m2) = *EMIN(mon);
         ...
         */
        m2->mtame = 0;
        if (tamedog(m2, (struct obj *) 0, FALSE, FALSE)) {
    ...
    mmflags_nht mmflags)
{
    ...
    *mtmp = cg.zeromonst; /* clear all entries in structure */
    ...
    if (ptr->msound == MS_NEMESIS && quest_info(MS_NEMESIS) == mndx)
        svq.quest_status.nemesis_m_id = mtmp->m_id;
    mtmp->mnum = mndx;
    ...
    case S_MIMIC:
        ...
        break;
    case S_COCKATRICE:
        if (gi.in_mklev && touch_petrifies(ptr)
                ...
                struct monst* creator)
{
    ...
    while (cnt--) {
        ...
        x = (creator == &gy.youmonst ? u.ux : creator->mx);
        y = (creator == &gy.youmonst ? u.uy : creator->my);
...
set_mimic_sym(struct monst *mtmp)
{
    ...
    if (roomno >= 0)
        rt = gi.in_mklev ? svr.rooms[roomno].rtype
                         : svr.rooms[roomno].orig_rtype;
    ...
    int *seencount)  /* secondary output */
{
    ...
    if (!bag || bag->otyp != BAG_OF_TRICKS) {
        ...
        seecount = create_critters(creatcnt, NULL, FALSE, &gy.youmonst);
        if (seecount) {
            if (seencount)
                *seencount += seecount;
            if (bag->dknown) {
