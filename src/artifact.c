    ...
    if (spfx & SPFX_WARN) {
        if (spec_m2(otmp)) {
            ...
            if (on) {
                ...
                if (is_mlet) {
                    gc.context.warntype.obj_mlet = type;
                }
                else {
                    gc.context.warntype.obj |= type;
                }
            } else {
                EWarn_of_mon &= ~wp_mask;
                if (is_mlet) {
                    gc.context.warntype.obj_mlet = 0;
                }
                else {
                    gc.context.warntype.obj &= ~type;
                }
	...
	case SMOKE_CLOUD: {
	    ...
	    char c;
            ...
            if (c == 'a') {
                ...
                gc.context.crystal.ball = obj;
                gc.context.crystal.o_id = obj->o_id;
                gc.context.crystal.looktime = rn1(5, 6); /* same as ball */
