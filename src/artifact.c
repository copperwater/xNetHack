...
staticfn int invoke_lightning_bolt(struct obj *) NONNULLARG1;
staticfn int invoke_smoke_cloud(struct obj *) NONNULLARG1;
    ...
    if (spfx & SPFX_WARN) {
        if (spec_m2(otmp)) {
            ...
            if (on) {
                ...
                if (is_mlet) {
                    svc.context.warntype.obj_mlet = type;
                }
                else {
                    svc.context.warntype.obj |= type;
                }
            } else {
                EWarn_of_mon &= ~wp_mask;
                if (is_mlet) {
                    svc.context.warntype.obj_mlet = 0;
                }
                else {
                    svc.context.warntype.obj &= ~type;
                }
...
invoke_healing(struct obj *obj)
{
    ...
    /* Blinded > creamed also catches permanent intrinsic blindness */
...
staticfn int
invoke_lightning_bolt(struct obj *obj UNUSED)
{
    struct obj* pseudo = mksobj(WAN_LIGHTNING, FALSE, FALSE);
    pseudo->blessed = pseudo->cursed = 0;
    /* type is a "spell of lightning bolt" which doesn't actually exist:
     * 10 + AD_ELEC - 1 */
...
staticfn int
invoke_smoke_cloud(struct obj *obj)
{
    ...
    if (c == 'a') {
        ...
        svc.context.crystal.ball = obj;
        svc.context.crystal.o_id = obj->o_id;
        svc.context.crystal.looktime = rn1(5, 6); /* same as ball */
...
arti_invoke(struct obj *obj)
{
    ...
    if (oart->inv_prop > LAST_PROP) {
        ...
        case LIGHTNING_BOLT: res = invoke_lightning_bolt(obj); break;
        case SMOKE_CLOUD: res = invoke_smoke_cloud(obj); break;
        default:
        ...
        case CONFLICT:
            if (on)
                 ...
                 * just prior to setting up gc.context.victual, there isn't any
...
retouch_object(struct obj **objp, /* might be destroyed or unintentionally dropped */
               ...
               boolean protected_by_gear) /* whether the player has gear that
                                           * will protect them from touching
                                           * *obj if it is a hated material */
{
    struct obj *obj = *objp;
...
/* like has_magic_key but for the Apple of Discord - only for hero */
struct obj *
uhave_magic_apple(void)
{
    ...
    for (otmp = gi.invent; otmp; otmp = nxtobj(otmp, otyp, FALSE)) {
