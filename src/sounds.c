...
staticfn boolean is_stormy_monster(struct monst *);
staticfn void pacify_with_words(struct monst *);
    ...
    if (!rn2(300)) {
        ...
        if (br && In_mines(&br->end2) && !In_mines(&u.uz)) {
            ...
            return;
                 ...
                 && svm.mvitals[PM_VLAD_THE_IMPALER].died == 0) {
...
domonnoise(struct monst *mtmp)
{
    ...
    if (mtmp->m_id == svq.quest_status.leader_m_id && msound > MS_ANIMAL)
        msound = MS_LEADER;
    if (mtmp->m_id == svq.quest_status.nemesis_m_id && msound > MS_ANIMAL)
        msound = MS_NEMESIS;
...
dochat(void)
{
    ...
    for (dx = -1; dx <= +1; dx++) {
        for (dy = -1; dy <= +1; dy++) {
             ...
             * effect when there's a monster but its domonnoise() does nothing,
             * but there's no good heuristic for that right now. */
