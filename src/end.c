...
# endif
#endif
staticfn boolean sentient_arise(int);
...
done2(void)
{
    ...
    if (abandon_tutorial
        || !paranoid_query(ParanoidQuit,
                    "Really quit? (This will end your game permanently!)")) {
#ifndef NO_SIGNAL
        (void) signal(SIGINT, (SIG_RET_TYPE) done1);
#endif
        clear_nhwindow(WIN_MESSAGE);
...
/* If the player is killed by something that allows their body to continue
 * existing as some kind of monster that's not a ghost, they might retain
 * their original mind and agency (and the game won't end after all). However,
 * this comes with a drawback - the new form has intrinsic unchanging, and dying
 * in that form will cause permanent death.
...
sentient_arise(int how)
{
    ...
    if (mon_nm < LOW_PM || (svm.mvitals[mon_nm].mvflags & G_GENOD)) {
        return FALSE;
    }
    ...
    if ((mon_nm == PM_GREEN_SLIME && !rn2(2)) ||
 ...
 */
void
format_monkiller(struct monst *mtmp)
{
    char buf[BUFSZ];
    ...
    time_t when) /* date+time at end of game */
{
    ...
    putstr(NHW_DUMPTXT, 0, "");
    putstr(0, ATR_HEADING, "Dungeon overview:");
    ...
    /* calculate score, before creating bones [container gold] */
    {
        ...
        if (how == ASCENDED) {
            if (u.ualign.type == u.ualignbase[A_ORIGINAL]) {
                ...
                Strcat(svk.killer.name, " (in dishonor)");
            }
        }
    }
