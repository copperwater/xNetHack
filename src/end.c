...
#endif
#endif
static boolean sentient_arise(int);
...
sentient_arise(int how)
{
    ...
    if (mon_nm < LOW_PM || (g.mvitals[mon_nm].mvflags & G_GENOD)) {
        return FALSE;
    }
...
done_in_by(struct monst *mtmp, int how)
{
    ...
    if (mptr->mlet == S_WRAITH)
        ...
        u.ugrave_arise = g.urace.zombienum;
...
done(int how)
{
    ...
    if (Lifesaved && (how <= GENOCIDED) && !nonliving(g.youmonst.data)) {
        pline("But wait...");
    ...
    /* calculate score, before creating bones [container gold] */
    {
        ...
        if (how == ASCENDED) {
            if (u.ualign.type == u.ualignbase[A_ORIGINAL]) {
                ...
                Strcat(g.killer.name, " (in dishonor)");
            }
        }
    }
