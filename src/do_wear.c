...
staticfn int wornmask_to_bodypart(int);
 ...
 */
void
cursed_gear_welds(struct obj* obj)
{
    if (obj->cursed) {
    ...
    if (Blind && !already_blind) {
        ...
        if (u.uroleplay.blind) {
            ...
            if (Blinded & INTRINSIC)
                HBlinded = 0L; /* this deliberately removes any INTRINSIC flags */
            You("can see!");
        }
    }
    if (changed) {
...
staticfn int
wornmask_to_bodypart(int wornmask)
{
