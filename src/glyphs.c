...
glyph_to_cmap(int glyph)
{
    ...
    else if (glyph_is_cmap_engraving(glyph)) {
        /* this one depends on whether it's in the first or second half of
         * engraving glyphs */
        if ((glyph - GLYPH_ENGRAVING_OFF) >= max_engraving_glyphtypes)
            return S_engrcorr;
        else
            return S_engroom;
    }
    else if (glyph_is_cmap_magicplatform(glyph))
        return S_magicplatform;
    else
