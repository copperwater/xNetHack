...
staticfn boolean mhitm_mgc_atk_negated(struct monst *, struct monst *,
                                     boolean) NONNULLPTRS;
staticfn boolean attack_check_conducts(struct obj *);
    ...
    struct obj *wep)    /* uwep for do_attack(), null for kick_monster() */
{
    ...
    if (mtmp->mundetected && !canseemon(mtmp)
        ...
        if (!tp_sensemon(mtmp) && !Detect_monsters) {
            ...
            else if (concealed_spot(mtmp, mtmp->mx, mtmp->my)) {
 ...
 */
staticfn boolean
attack_check_conducts(struct obj* wep)
{
    ...
    if (!said_yes_to_attack && !u.uconduct.killer && !svc.context.forcefight
        && svm.moves > 100) {
...
hmon_hitmon_barehands(struct _hitmon_data *hmd, struct monst *mon)
{
     ...
     * rings or gloves made of a hated material.
     * Note:  rings are worn under gloves, so you don't get both bonuses, and
     * two silver rings don't give double bonus. When making only one hit, both
     * rings are checked (backwards compatibility => playability), but when
     * making two hits, only the ring on the hand making the attack is checked.
     */
    switch (hmd->twohits) {
    case 0: /* only one hit being attempted; a hated ring on either hand
             * applies but having hated rings on both is same as just one */
        ringmask = (W_RINGL | W_RINGR);
        break;
    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    if (noncorporeal(hmd->mdat) && !shade_glare(obj))
        hmd->dmg = 0;
    else
        hmd->dmg = rnd(2);
    ...
    if (!hmd->thrown && obj == uwep && obj->otyp == THIEFSTONE && obj->blessed
    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    if (!hmd->train_weapon_skill || mon == u.ustuck || u.twoweap
        ...
        ; /* no special bonuses */
    } else if (hmd->mdat->mlet == S_VAMPIRE && obj->material == WOOD
               && (objects[obj->otyp].oc_dir & PIERCE)
               && which_armor(mon, W_ARM) == 0 && hmd->dieroll == 1) {
        /* Critical hit with a piercing wooden weapon = staked through the
         * heart! */
        hmd->dmg = mon->mhp + 100;
        if (Hallucination)
    ...
    if ((obj->opoisoned || permapoisoned(obj)) && is_poisonable(obj)) {
        /* Since non-thrown poison sources do more damage, they would be way too
         * powerful to poison on each hit - so artificially limit the
         * opportunity for it to score a hit.
         * However, we don't want to only cause alignment penalties for using
         * Grimtooth when it actually causes poison: the intent was to use a
         * poisoned weapon whether or not it actually poisoned anything.
         * Grimtooth's 1/4 chance of poisoning is based on dieroll, which
         * provides a consistent way to make sure the poison damage is only
         * dealt when the special message is delivered.
         * For other (currently nonexistent) poisoned melee weapons, using
         * dieroll should still work fine. */
        if (hmd->thrown == HMON_THROWN || hmd->dieroll <= 5) {
            hmd->ispoisoned = TRUE;
        }
    }
    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    hmd->mdat = mon->data;
    hmd->dmg = (noncorporeal(hmd->mdat)) ? 0 : 1;
}
...
#define useup_eggs(o)                    \
    ...
    case EGG: {
        ...
        if (ismnum(obj->corpsenm)
            ...
            if (strstri(pmname(mon->data, Mgender(mon)), "devil"))
    ...
    struct obj *obj)    /* obj can be NULL */
{
    ...
    if (!obj) { /* attack with bare hands */
        ...
        /* attacking with non-weapons */
            ...
            if (noncorporeal(hmd->mdat) && !shade_aware(obj)) {
                hmd->dmg = 0;
            } else {
    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    if (resists_poison(mon))
        ...
        hmd->dmg += (hmd->thrown == HMON_THROWN ? rnd(6) : rn1(10, 6));
}
    ...
    int dieroll)
{
    ...
    hmd.artimsg = ARTIFACTHIT_NOMSG;
    hmd.defer_breakwep = FALSE;
    ...
    if (hmd.dmg < 1) {
        ...
        hmd.dmg = (hmd.get_dmg_bonus && !noncorporeal(hmd.mdat)) ? 1 : 0;
        if (noncorporeal(hmd.mdat) && !hmd.hittxt
    ...
    /* already_killed won't apply for Trollsbane */
...
demonpet(void)
{
    ...
    if ((dtmp = makemon(pm, u.ux, u.uy, NO_MM_FLAGS)) != 0)
        (void) tamedog(dtmp, (struct obj *) 0, FALSE, FALSE);
    exercise(A_WIS, TRUE);
}
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (completelyrots(pd)) {
            You("rot!");
            /* KMH -- this is okay with unchanging */
            strcpy(svk.killer.name, "rotted away");
            svk.killer.format = NO_KILLER_PREFIX;
            rehumanize();
            return;
        }
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!mhitm_mgc_atk_negated(magr, mdef, TRUE)) {
            ...
            if (completelyburns(pd)) { /* paper or straw golem */
                ...
                /* KMH -- this is okay with unchanging */
                Strcpy(svk.killer.name, "immolated");
                svk.killer.format = NO_KILLER_PREFIX;
                rehumanize();
                return;
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!magr->mcan && !rn2(10)) {
            ...
            if (u.umonnum == PM_CLAY_GOLEM) {
                ...
                /* KMH -- this is okay with unchanging */
                Sprintf(svk.killer.name, "identity theft");
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (magr->data == &mons[PM_GHOST]) {
            mhm->damage = 0;
            boolean couldspot = canspotmon(magr);
            if (gm.multi < 0) { /* already paralyzed; ghost won't appear */
                return;
            }
            if (magr->minvis) {
                magr->minvis = 0;
                ...
                if (canseemon(magr) && !Unaware) {
                    ...
                    if (!couldspot) {
                        ...
                        * turned visible */
                        ...
                        pline("%s becomes visible!", Monnam(magr));
                        You("aren't scared.");
                    }
                }
            }
        ...
        if (gm.multi >= 0 && !rn2(3)
            ...
            make_paralyzed(rnd(10), !msg_given, (const char *) 0);
        }
    } else {
        /* mhitm */
...
void
mhitm_ad_pits(struct monst *magr, struct attack *mattk,
              struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        /* uhitm */
    ...
    if (!create_pit_under(mdef, magr)) {
        mhm->damage = 0;
        if (mdef != &gy.youmonst && DEADMONSTER(mdef)) { /* killed by pit */
            mhm->hitflags = M_ATTK_DEF_DIED;
        }
    }
}
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (noncorporeal(pd)) {
            mhm->damage = 0;
            if (!mhm->specialdmg)
                impossible("bad noncorporeal attack function flow?");
        }
        ...
        if (mattk->aatyp == AT_HUGS && !sticks(pd)) {
            ...
            if (magr->data == &mons[PM_YEENOGHU] && !rn2(10) && uwep
                && !uwep->cursed) {
                xint8 dx = u.ux - magr->mx;
                xint8 dy = u.uy - magr->my;
                int dir;
        ...
        if (shade_miss(magr, mdef, mwep, FALSE, vis)) {
            ...
            if (mwep->oartifact) {
                ...
                artimsg = artifact_hit(magr, mdef, mwep, &mhm->damage,
                                       mhm->dieroll);
                if (artimsg == ARTIFACTHIT_NOMSG) {
                    if (gv.vis)
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!(uwep && (uwep->oclass == WEAPON_CLASS || is_weptool(uwep)))
            ...
            pline_mon(magr, "%s touches you!  (I hope you don't mind.)",
                      Monnam(magr));
            if (Upolyd) {
                u.mh += rnd(7);
                if (!rn2(7)) {
...
gulpum(struct monst *mdef, struct attack *mattk)
{
    ...
    if (!(u_digest && u.uhunger >= 1500) && !u.uswallow) {
        ...
        if (fatal_gulp && !is_rider(pd)) { /* petrification */
            ...
            case AD_FIRE:
                if (rn2(2)) {
                    boolean steamy = (gy.youmonst.data ==
...
hmonas(struct monst *mon)
{
    ...
    /* with just one touch/claw/weapon attack, both rings matter;
       ...
       whether ring causes successful hit */
    for (i = 0; i < NATTK; i++) {
        sum[i] = M_ATTK_MISS;
 ...
 use_weapon:
        ...
        /*weaponless:*/
            ...
            if (dhit) {
                ...
                long contact_slots = attack_contact_slots(&gy.youmonst,
                                                          mattk->aatyp);
                ...
                case AT_TUCH:
                    ...
                    /* decide if material-hater will be hit by ring(s);
                    ...
                    odd_claw = !odd_claw;
                    /* xNetHack: other pieces of gear such as helms can hit the
                     * monster, so the special_dmgval call is moved to after
                     * this switch. Here we first suppress ring slots, then
                     * re-add them as necessary. W_ARMG should already have been
                     * set by attack_contact_slots and isn't modified here. */
                    contact_slots &= ~(W_RINGL | W_RINGR);
                    if (!multi_claw || odd_claw)
                        contact_slots |= W_RINGL;
                    if (!multi_claw || !odd_claw)
                        contact_slots |= W_RINGR;
                    break;
                case AT_TENT:
                ...
                specialdmg = special_dmgval(&gy.youmonst, mon, contact_slots,
                                            &hated_obj);
        ...
        if (dhit == -1) {
            ...
            Sprintf(svk.killer.name, "blew %sself up", uhim());
            svk.killer.format = NO_KILLER_PREFIX;
            rehumanize();
        }
        if (sum[i] == M_ATTK_DEF_DIED) {
            ...
            if (ptr == &mons[PM_FLOATING_EYE]) {
                ...
                if (mon->mcansee) {
                    ...
                    if (reflectsrc) {
                        ...
                        make_paralyzed(tmp, FALSE, (const char *) 0);
                    }
                } else {
...
item_catches_drain(struct monst *mdef)
{
    ...
    otmp = (mdef == &gy.youmonst) ? gi.invent : mdef->minvent;
    ...
    if (interceptor) {
        ...
        drain_item(interceptor, !svc.context.mon_moving);
    ...
    struct obj *otmp) /* source of flash */
{
    ...
    if (M_AP_TYPE(mtmp) != M_AP_NOTHING) {
        ...
        wakeup(mtmp, FALSE, TRUE); /* -> seemimic() -> newsym(); also calls
